...
// دریافت همه کت شلوارها (بدون صفحه‌بندی - برای استفاده در مکان‌های خاص)
export async function getSuits() {
    try {
        const { data, error } = await supabase
            .from("suits")
            .select(
                "id,name,price,discount,description,image,fabric,category,product_type",
            )
            .order("id");
...
export async function getSuitsPaginated(page = 1, limit = 6, filter = "all") {
    try {
        ...
        // اعمال فیلتر
        if (filter !== "all") {
            // ابتدا بررسی کن که فیلتر روی category است یا product_type
...
export async function getSuit(id) {
    try {
        ...
        const { data, error } = await supabase
            ...
            .eq("id", id)
            .single(); // این خط در صورت نبودن رکورد، خطا می‌دهد

        if (error) {
            // اگر خطا "Row not found" یا مشابه بود، به سادگی null برگردانید
...
// دریافت محصولات بر اساس نوع (برای فیلترهای palto, shirt, vest)
export async function getSuitsByType(productType, page = 1, limit = 6) {
    try {
        const start = (page - 1) * limit;
        const end = start + limit - 1;
        ...
        console.error("Error in getSuitsByType:", error);
        throw new Error("Suits could not be loaded");
    }
}
